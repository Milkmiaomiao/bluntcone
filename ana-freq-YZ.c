...
//对流场保存的截面时间序列数据 eg：Savedata-YZ00?.dat 进行傅里叶变换得到频谱(有量纲 spectrum-?.dat 和无量纲 findspectrum-?.dat )
//保存时间平均后的截面信息 eg：YZplane-time-average-?.dat 方便进行展向空间关联
//保存一个点的物理量随时间(无量纲)的变化序列 eg：time-data-?.dat
//根据无量纲的频谱挑选需要的频率进行傅里叶变换(需要再运行一次程序)，得到该频率在流场中的空间分布 eg：spectrum2d-?.dat
//输出所提取无量纲频率的型函数

//=======================================================================
//  2022.3.20  增加了展向脉动关联和流向脉动关联（XZ.c）
//=======================================================================
...
#define mp 1000     //100个时间序列，可以按需求更改 
#define k0 1    //两点关联的壁面法向网格点                         
//#define np 0       //第几个YZ截面，对应程序调用中的i0
//#define nz_myid 8   //输出DMD数据截面时只输出前8个分块的数据
//#define dt0 0.10*(9.35376e-7)   //根据保存截面所隔时间步决定
int nxpoints[] = {896,1152,1408,1664,1920,2176,2432,2688,2944,3200,3456,3712,3968,4480,5120,5760};  //x对应的编号 
//616,923,1231,1539,1846,2154,2462,2769,3077,3385,3692,4000,4308,4615       
...
int np, ib, ie, kb, ke, mp_end, nstep;   //np:第几个YZ截面，对应程序调用中的i0  mp:多少个时间序列，可以按需求更改
int nz_myid;
...
//double *dtmpRaa, *utmpRaa, *vtmpRaa, *wtmpRaa, *TtmpRaa, *PtmpRaa;
double tt[mp];  //读取mp个时间序列，tt存放无量纲时尖
...
double w1d0[mp], w1d1[mp], w1d2[mp];
double *p2d0, *u2d1, *T2d1;
/*double T1d[2][mp];
double d1d[2][mp];
double u1d[2][mp];*/
double deltx = 2.0 * PI / mp;

//======================这里也需要修改，根据保存截面所隔时间步决定=====================================
...
void Write_Savedata_timeaverage(int i0);
void Compute_frequancy(double U1d[], int i0, int i1, char* Uf );   //i0为选取的YZ截面编号，i1用于区分输出的物理量及其位置，i1=0为壁面处压力，i1=1为边界层附近压力
...
void Compute_fourier2d(double* U2d, int i0, int i1, char* Uf);
...
//====================调用函数的时候需要修改参数=======================
        ...
        //Write_Savedata_timeaverage(np);
        //DMD_XZ_coordinate(np);
        /*Compute_frequancy(P1d0, np, 0, fP);
        Compute_frequancy(P1d1, np, 1, fP);
        Compute_frequancy(P1d2, np, 2, fP);
        Compute_frequancy(T1d0, np, 0, fT);
        Compute_frequancy(T1d1, np, 1, fT);
        Compute_frequancy(T1d2, np, 2, fT);
        Compute_frequancy(d1d0, np, 0, fd);
        Compute_frequancy(d1d1, np, 1, fd);
        Compute_frequancy(d1d2, np, 2, fd);
        Compute_frequancy(u1d0, np, 0, fu);
        Compute_frequancy(u1d1, np, 1, fu);
        Compute_frequancy(u1d2, np, 2, fu);*/
        Compute_fourier2d(p2d0, np, 0, fP);
        Compute_fourier2d(u2d1, np, 1, fu);
        Compute_fourier2d(T2d1, np, 1, fT);
        //Compute_Raa(np);
        //Compute_frequancy_2dYZ( np , 96); //第二个参数96为无量纲频率wk
        //Compute_frequancy_2dYZ( np , 30);
...
void Read_parameter(){
    if(my_id == 0){
        ...
        printf("np is %d\nib is %d\nie is %d\nkb is %d\nke is %d\nmp is %d\nnstep is %d\nnz_myid is %d\ndt0 is %lf\n",
              np, ib, ie, kb, ke, mp_end, nstep, nz_myid, dt0);
        /*fgets(str, 2000, fp3);
...
void Data_malloc(){
    ...
    p2d0 = (double*)malloc(mp * ny * sizeof(double));
    Malloc_Judge(p2d0);

    u2d1 = (double*)malloc(mp * ny * sizeof(double));
    Malloc_Judge(u2d1);

    T2d1 = (double*)malloc(mp * ny * sizeof(double));
    Malloc_Judge(T2d1);


    head = (int*)malloc(5 * sizeof(int));

    dtmp = (double*)malloc( mp * ny * NZ * sizeof(double));
    Malloc_Judge(dtmp);

    utmp = (double*)malloc( mp * ny * NZ * sizeof(double));
    Malloc_Judge(utmp);
    
    vtmp = (double*)malloc( mp * ny * NZ * sizeof(double));
    Malloc_Judge(vtmp);

    wtmp = (double*)malloc( mp * ny * NZ * sizeof(double));
    Malloc_Judge(wtmp);

    Ttmp = (double*)malloc( mp * ny * NZ * sizeof(double));
    Malloc_Judge(Ttmp);

    Ptmp = (double*)malloc( mp * ny * NZ * sizeof(double));
    Malloc_Judge(Ptmp);

    /*dtmpRaa = (double*)malloc( mp * ny * nz * sizeof(double));
    Malloc_Judge(dtmpRaa);

    utmpRaa = (double*)malloc( mp * ny * nz * sizeof(double));
    Malloc_Judge(utmpRaa);
    
    vtmpRaa = (double*)malloc( mp * ny * nz * sizeof(double));
    Malloc_Judge(vtmpRaa);

    wtmpRaa = (double*)malloc( mp * ny * nz * sizeof(double));
    Malloc_Judge(wtmpRaa);

    TtmpRaa = (double*)malloc( mp * ny * nz * sizeof(double));
    Malloc_Judge(TtmpRaa);

    PtmpRaa = (double*)malloc( mp * ny * nz * sizeof(double));
    Malloc_Judge(PtmpRaa);*/


    pd1 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pd1);

    pu1 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pu1);

    pv1 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pv1);

    pw1 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pw1);

    pT1 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pT1);

    pP1 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pP1);

    pd0 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pd0);
    memset((void*)pd0, 0, ny * NZ * sizeof(double));

    pu0 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pu0);
    memset((void*)pu0, 0, ny * NZ * sizeof(double));

    pv0 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pv0);
    memset((void*)pv0, 0, ny * NZ * sizeof(double));

    pw0 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pw0);
    memset((void*)pw0, 0, ny * NZ * sizeof(double));

    pT0 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pT0);
    memset((void*)pT0, 0, ny * NZ * sizeof(double));

    pP0 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pP0);
    memset((void*)pP0, 0, ny * NZ * sizeof(double));

    pdrms = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pdrms);
    memset((void*)pdrms, 0, ny * NZ * sizeof(double));

    purms = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(purms);
    memset((void*)purms, 0, ny * NZ * sizeof(double));

    pvrms = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pvrms);
    memset((void*)pvrms, 0, ny * NZ * sizeof(double));

    pwrms = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pwrms);
    memset((void*)pwrms, 0, ny * NZ * sizeof(double));

    pTrms = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pTrms);
    memset((void*)pTrms, 0, ny * NZ * sizeof(double));

    pPrms = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pPrms);
    memset((void*)pPrms, 0, ny * NZ * sizeof(double));

    pd2 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pd2);
    memset((void*)pd2, 0, ny * NZ * sizeof(double));

    pu2 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pu2);
    memset((void*)pu2, 0, ny * NZ * sizeof(double));

    pv2 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pv2);
    memset((void*)pv2, 0, ny * NZ * sizeof(double));

    pw2 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pw2);
    memset((void*)pw2, 0, ny * NZ * sizeof(double));

    pT2 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pT2);
    memset((void*)pT2, 0, ny * NZ * sizeof(double));

    pP2 = (double*)malloc( ny * NZ * sizeof(double));
    Malloc_Judge(pP2);
    memset((void*)pP2, 0, ny * NZ * sizeof(double));
...
void Read_Savedata(int i0){
    ...
    double (*d0)[ny] = (double (*)[ny])(pd0);
    double (*u0)[ny] = (double (*)[ny])(pu0);
    double (*v0)[ny] = (double (*)[ny])(pv0);
    double (*w0)[ny] = (double (*)[ny])(pw0);
    double (*T0)[ny] = (double (*)[ny])(pT0);
    double (*P0)[ny] = (double (*)[ny])(pP0);

    double (*drms)[ny] = (double (*)[ny])(pdrms);
    double (*urms)[ny] = (double (*)[ny])(purms);
    double (*vrms)[ny] = (double (*)[ny])(pvrms);
    double (*wrms)[ny] = (double (*)[ny])(pwrms);
    double (*Trms)[ny] = (double (*)[ny])(pTrms);
    double (*Prms)[ny] = (double (*)[ny])(pPrms);

    double (*pp2d0)[ny] = (double(*)[ny])(p2d0);
    double (*pu2d1)[ny] = (double(*)[ny])(u2d1);
    double (*pT2d1)[ny] = (double(*)[ny])(T2d1);
    ...
    pP_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pd0_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pu0_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pv0_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pw0_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pT0_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pP0_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pdrms_buff  = (double*)malloc(ny * NZ * sizeof(double));
    purms_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pvrms_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pwrms_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pTrms_buff  = (double*)malloc(ny * NZ * sizeof(double));
    pPrms_buff  = (double*)malloc(ny * NZ * sizeof(double));
    ...
    double (*ppP_buff)[ny]  = (double(*)[ny])pP_buff;
    double (*ppd0_buff)[ny]  = (double(*)[ny])pd0_buff;
    double (*ppu0_buff)[ny]  = (double(*)[ny])pu0_buff;
    double (*ppv0_buff)[ny]  = (double(*)[ny])pv0_buff;
    double (*ppw0_buff)[ny]  = (double(*)[ny])pw0_buff;
    double (*ppT0_buff)[ny]  = (double(*)[ny])pT0_buff;
    double (*ppP0_buff)[ny]  = (double(*)[ny])pP0_buff;
    double (*ppdrms_buff)[ny]  = (double(*)[ny])pdrms_buff;
    double (*ppurms_buff)[ny]  = (double(*)[ny])purms_buff;
    double (*ppvrms_buff)[ny]  = (double(*)[ny])pvrms_buff;
    double (*ppwrms_buff)[ny]  = (double(*)[ny])pwrms_buff;
    double (*ppTrms_buff)[ny]  = (double(*)[ny])pTrms_buff;
    double (*ppPrms_buff)[ny]  = (double(*)[ny])pPrms_buff;
    ...
    char filename1[120];
    char filename2[120];
    double tmpP1[mp], tmpT1[mp], tmpd1[mp], tmpu1[mp], tmpw1[mp];
    double tmpP2[mp], tmpT2[mp], tmpd2[mp], tmpu2[mp], tmpw2[mp];
    double tmpP0[mp];
    double tmp2du1[mp][ny], tmp2dT1[mp][ny];
    double tmp;
    ...
    sprintf(fp_name, "Savedata-YZ%03d.dat", i0);
    /*sprintf(fp_name1, "DMD-data-YZ%03d-T.dat", i0);
    sprintf(fp_name2, "DMD-data-YZ%03d-P.dat", i0);*/
    ...
    for(int i = 0; i < mp; i++ ){                      //读取mp个时间序列
        ...
        for(int k = 0; k < NZ; k++){
            for(int j = 0; j < ny; j++){
                ...
                xx3d_buff[k][j] = xx3d[k][j][nxpoints[i0]];      //设置缓冲数组防止数据交换时覆盖掉
                yy3d_buff[k][j] = yy3d[k][j][nxpoints[i0]];
                zz3d_buff[k][j] = zz3d[k][j][nxpoints[i0]];
                ppd_buff[k][j]  = d[k][j];
                ppu_buff[k][j]  = u[k][j];             
                ppv_buff[k][j]  = v[k][j];
                ppw_buff[k][j]  = w[k][j];
                ...
                MPI_Bcast(tmpw1, mp, MPI_DOUBLE, id1, MPI_COMM_WORLD);

                MPI_Bcast(tmp2du1, mp*ny, MPI_DOUBLE, id1, MPI_COMM_WORLD);
                MPI_Bcast(tmp2dT1, mp*ny, MPI_DOUBLE, id1, MPI_COMM_WORLD);
                ...
                MPI_Bcast(tmpw2, mp, MPI_DOUBLE, id2, MPI_COMM_WORLD);
                ...
                if(my_id != id1){
                        ...
                        w1d1[i] = tmpw1[i];
                    for(int j = 0; j < ny; j++){
                        pu2d1[i][j] = tmp2du1[n1][j];
                        pT2d1[i][j] = tmp2dT1[n1][j]; 
                    }
                }
                if(my_id != id2){
        ...
        /*if(my_id < nz_myid){
            ...
            //if(my_id == 0) printf("compute T2d P2d OK\n");                                                       
            

            MPI_File_seek(tmp_file1, sizeof(int), MPI_SEEK_CUR);
            //if(my_id == 0) printf("seek OK\n"); 
	        MPI_File_write(tmp_file1, &Istep, 1, MPI_INT, &status);//_all
            //if(my_id == 0) printf("Istep OK\n");             
            MPI_File_write(tmp_file1, &tt[i], 1, MPI_DOUBLE, &status);//_all
            //if(my_id == 0) printf("tt OK\n");
            MPI_File_seek(tmp_file1, sizeof(int), MPI_SEEK_CUR);
            //if(my_id == 0) printf("seek OK\n");
	        ...
	        MPI_File_write(tmp_file2, &Istep, 1, MPI_INT, &status);  //_all           
            MPI_File_write(tmp_file2, &tt[i], 1, MPI_DOUBLE, &status);//_all
            MPI_File_seek(tmp_file2, sizeof(int), MPI_SEEK_CUR);
    ...
    sprintf(filename1, "time1d-YZ%03d.dat",i0);
    ...
    for(int n = 0; n < n_processe; n++){
            ...
            if(my_id != 0){
                ...
                MPI_Send(pP_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pd0_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pu0_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pv0_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pw0_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pT0_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pP0_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pdrms_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(purms_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pvrms_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pwrms_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pTrms_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pPrms_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
            }
            ...
            if(my_id != n_processe-1){
                ...
                MPI_Recv(pP_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pd0_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pu0_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pv0_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pw0_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pT0_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pP0_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pdrms_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(purms_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pvrms_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pwrms_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pTrms_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pPrms_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
            }
    }*/
...
void DMD_XZ_coordinate(int i0){
    ...
    for(int k = 0; k < NZ; k++){
        for(int i = 0; i < ni; i++){
            ...
            xx2d_buff[k][i] = xx3d[k][i1][nxpoints[i0]];      //设置缓冲数组防止数据交换时覆盖掉
            yy2d_buff[k][i] = yy3d[k][i1][nxpoints[i0]];
            zz2d_buff[k][i] = zz3d[k][i1][nxpoints[i0]];
    ...
    //for(int i = 0; i < mp; i++ ){                      //mp个时间序列
        ...
        for(int k = 0; k < NZ; k++){
            ...
            MPI_File_write(tmp_file, y2d_buff+num*k, num,  MPI_DOUBLE, &status);
                ...
                for(int k = 0; k < NPZ[n]; k++){
                    for(int j = 0; j < ni; j++){
                            ...
                            fprintf(fp, "%15.6f%15.6f\n", 
                                yy2d_buff[k][j], zz2d_buff[k][j]);  //, yy2d_buff[k][j]
...
void Compute_frequancy(double U1d[], int i0, int i1, char* Uf){
    double (*xx3d)[ny][nx] = (double (*)[ny][nx])(x3d);
    double (*yy3d)[ny][nx] = (double (*)[ny][nx])(y3d);
    double (*zz3d)[ny][nx] = (double (*)[ny][nx])(z3d);
    ...
    //fgets(strU, 4, U1d);
    sprintf(filename, "spectrumYZ-%03d-%d-%s.dat", i0, i1, Uf);   //读取不同截面的时候需要改
    ...
    if(my_id == 0){
        ...
        for(int i = 0; i < 1000; i++){
            ...
            omeg = i /(mp * dt0)/1000.0;   //这里omeg即为频率f，单位KHz

            St = omeg * 0.001/1069.1;   //St=fl/U , l=1mm, U=1069.1 m/s

            fprintf(fp, "%d%15.6f%15.6f%18.14f\n", i, omeg, St, Uamplitude[i]);
        }
        fclose(fp);
    }
...
void Compute_fourier2d(double* U2d, int i0, int i1, char* Uf){                                     //2维FT变换，时间方向以及空间展向
    double (*xx3d)[ny][nx] = (double (*)[ny][nx])(x3d);
    double (*yy3d)[ny][nx] = (double (*)[ny][nx])(y3d);
    double (*zz3d)[ny][nx] = (double (*)[ny][nx])(z3d);

    double (*pU2d)[ny] = (double (*)[ny])(U2d);
    ...
    double complex CI = 0.0 + 1.0 * I;       //复数
    double x, y;
    double Uamplitude[500][100];  //纬度等于无量纲频率omeg的数目
    double delty = 2.0 * PI / (ny-1);
    double dt1 = sqrt( pow(yy3d[0][1][nxpoints[i0]]-yy3d[0][0][nxpoints[i0]],2) + pow(zz3d[0][1][nxpoints[i0]]-zz3d[0][0][nxpoints[i0]],2) ) ;
    ...
    if(my_id == 0){
        for(int i = 0; i < mp; i++){
            for(int j = 0; j < ny; j++){
                favP0 = favP0 + pU2d[i][j];    //所有时间所有空间点处的物理量相加
            }
        }
        printf("1\n");
        //for(int i = 0; i < mp; i++){    
            //for(int j = 0; j < ny; j++){
                favP0 = favP0 / (mp*ny);                //系综平均
            //}
        //}
        printf("favP0 = %lf\n", favP0);
        for(int i = 0; i < mp; i++){
            for(int j = 0; j < ny; j++){
                pU2d[i][j] = pU2d[i][j] - favP0;    //脉动值
            }//printf("U1d = %15.6f\n", U1d[i]);
        }
        printf("compute average data and disturbance data OK\n");                                                                             

        for(int i = 0; i < 500; i++){
            for(int j = 0; j < 100; j++){
                omeg = i;
                beta = j;                           //无量纲频率从0到999变化
                //printf("omeg = %lf, beta = %lf \n", omeg, beta);
                
                UC = 0.0 + 0.0 * I ; //每次循环都将UC更新为0，防止UC在原有数值上累加
                for(int i1 = 0; i1 < mp; i1++ ){ 
                    for(int j1 = 0; j1 < ny; j1++ ){
                        x = i1 * deltx;
                        y = j1 * delty;
                        UC = UC + pU2d[i1][j1] * cexp(CI * (omeg * x + beta * y)); // * cexp(CI * beta * y);      //做傅里叶变换得到幅值UC
                        //UC = UC + pU2d[i1][j1] * cexp(CI * omeg * x ) * cexp(CI * beta * y);
                    }
                }
                Uamplitude[i][j] = 2.0 * cabs(UC) / (mp * ny);       //since abs(f(w))=abs(f(-w)), so the amplitude is two times !!!
                //printf("Uamplitude = %lf \n",Uamplitude[i][j] );
                /*double complex UC = 0.0 + 0.0 * I ;
                for(int j1 = 0; j1 < ny; j1++ ){
                    y = j1 * delty;
                    UC = UC + U2d[i][j1] * cexp(CI * beta * y);
                }
                Uamplitude[i][j] = 2.0 * cabs(UC) / ny;*/
            }//printf("Uamplitude = %15.6f\n", Uamplitude[i]);
        }
        printf("3\n");
    }
    ...
    //fgets(strU, 4, U1d);
    sprintf(filename, "spectrumYZ2d-%03d-%d-%s.dat", (int)(xx3d[0][0][nxpoints[i0]]), i1, Uf);   //读取不同截面的时候需要改
    ...
    if(my_id == 0){
        ...
        fprintf(fp, "variables= omeg*, beta*, omeg, beta, St, %s \n", Uf);
        fprintf(fp, "zone i=%d ,j=%d \n",  500, 100);
        
        printf("dt0 = %15.6f, dt1 = %15.6f\n", dt0, dt1);
        ...
        for(int i = 0; i < 500; i++){
            for(int j = 0; j < 100; j++){
                ...
                beta = j /((ny-1) * dt1)/1000.0;

                St = omeg * 0.001/1069.1;   //St=fl/U , l=1mm, U=1069.1 m/s

                fprintf(fp, "%5.2d%5.2d%15.6f%15.6f%15.6f%15.6f\n", i, j, omeg, beta, St, Uamplitude[i][j]);
            }
        }
        fclose(fp);
    }
...
void Compute_frequancy_2dYZ( int i0 ,int wk){
    double (*xx3d)[ny][nx] = (double (*)[ny][nx])(x3d);
    double (*yy3d)[ny][nx] = (double (*)[ny][nx])(y3d);
    double (*zz3d)[ny][nx] = (double (*)[ny][nx])(z3d);
    ...
    double (*d2)[ny] = (double (*)[ny])(pd2);
    double (*u2)[ny] = (double (*)[ny])(pu2);
    double (*v2)[ny] = (double (*)[ny])(pv2);
    double (*w2)[ny] = (double (*)[ny])(pw2);
    double (*T2)[ny] = (double (*)[ny])(pT2);
    double (*P2)[ny] = (double (*)[ny])(pP2);
    ...
    y3d_buff = (double*)malloc(ny * NZ * sizeof(double));
    z3d_buff = (double*)malloc(ny * NZ * sizeof(double));
    pd2_buff = (double*)malloc(ny * NZ * sizeof(double));
    pu2_buff = (double*)malloc(ny * NZ * sizeof(double));
    pv2_buff = (double*)malloc(ny * NZ * sizeof(double));
    pw2_buff = (double*)malloc(ny * NZ * sizeof(double));
    pT2_buff = (double*)malloc(ny * NZ * sizeof(double));
    pP2_buff = (double*)malloc(ny * NZ * sizeof(double));

    double (*xx3d_buff)[ny] = (double(*)[ny])x3d_buff;
    double (*yy3d_buff)[ny] = (double(*)[ny])y3d_buff;
    double (*zz3d_buff)[ny] = (double(*)[ny])z3d_buff;
    double (*ppd2_buff)[ny] = (double(*)[ny])pd2_buff;
    double (*ppu2_buff)[ny] = (double(*)[ny])pu2_buff;
    double (*ppv2_buff)[ny] = (double(*)[ny])pv2_buff;
    double (*ppw2_buff)[ny] = (double(*)[ny])pw2_buff;
    double (*ppT2_buff)[ny] = (double(*)[ny])pT2_buff;
    double (*ppP2_buff)[ny] = (double(*)[ny])pP2_buff;
   
    int num =  ny;
    int num_byte =  ny * sizeof(double);
    ...
    sprintf(filename, "spectrum2dYZ-%03d-%d.dat", i0, wk);   //读取不同截面的时候需要改,i0表示读取第几个XZ截面，i1表示物理量及其对应的位置
    //sprintf(filename1, "shapefunction-%03d-%d.dat", i0, wk);
    ...
    if(my_id == 0){
        ...
        for(int k = 0; k < NZ; k++){
            for(int j = 0; j < ny; j++){
                xx3d_buff[k][j] = xx3d[k][j][nxpoints[i0]];
                yy3d_buff[k][j] = yy3d[k][j][nxpoints[i0]];
                zz3d_buff[k][j] = zz3d[k][j][nxpoints[i0]];
    ...
    for(int n = 0; n < n_processe; n++){
            ...
            if(my_id != 0){
                ...
                MPI_Send(y3d_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(z3d_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pd2_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pu2_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pv2_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pw2_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pT2_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
                MPI_Send(pP2_buff, ny*NPZ[my_id], MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
            }
            ...
            if(my_id != n_processe-1){
                ...
                MPI_Recv(y3d_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(z3d_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pd2_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pu2_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pv2_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pw2_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pT2_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(pP2_buff, ny*NPZ[my_id+1], MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
            }
    }
...
void Compute_Raa( int i0 ){
    ...
    double (*pdtmp)[NZ][ny] = (double (*)[NZ][ny])(dtmp);
    double (*putmp)[NZ][ny] = (double (*)[NZ][ny])(utmp);
    double (*pvtmp)[NZ][ny] = (double (*)[NZ][ny])(vtmp);
    double (*pwtmp)[NZ][ny] = (double (*)[NZ][ny])(wtmp);
    double (*pTtmp)[NZ][ny] = (double (*)[NZ][ny])(Ttmp);
    double (*pPtmp)[NZ][ny] = (double (*)[NZ][ny])(Ptmp);
    ...
    double (*Rdd) = (double(*))malloc(ny*sizeof(double));
    double (*Ruu) = (double(*))malloc(ny*sizeof(double));
    double (*Rvv) = (double(*))malloc(ny*sizeof(double));
    double (*Rww) = (double(*))malloc(ny*sizeof(double));
    double (*RTT) = (double(*))malloc(ny*sizeof(double));
    double (*RPP) = (double(*))malloc(ny*sizeof(double));
    ...
    sprintf(filename, "RaaYZ-%04d.dat", (int)(xx3d[0][0][nxpoints[i0]]) );

    if(my_id == id){
    //printf("1\n");
    for(int kr=0; kr<ny; kr++){
        for(int k=0; k<ny; k++){
            for(int m=0; m<mp; m++){
                if(k+kr > ny-1){
                    kr0 = kr - ny+1;
    ...
    printf("write RaaYZ data ...\n");
        ...
        fprintf(fp, "variables=kr,Rdd,Ruu,Rvv,Rww,RTT,Rpp \n");
        
        for(int kr = 0; kr<ny/2; kr++){
            fprintf(fp, "%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f\n", 
                    kr*360.0/5800,Rdd[kr]/Rdd[0],Ruu[kr]/Ruu[0],Rvv[kr]/Rvv[0],Rww[kr]/Rww[0],RTT[kr]/RTT[0],RPP[kr]/RPP[0]);
...
void get_data_yz(double (*data2d)[nz][ny], double (*data3d)[NZ][ny]){
    double *pdata_buff = (double*)malloc(ny*NZ*sizeof(double));
    double (*data_buff)[NZ][ny] = (double (*)[NZ][ny])(pdata_buff);

for(int m=0; m<mp; m++){
    for(int k=0; k<NZ; k++){
        for(int i=0; i<ny; i++){
            data_buff[m][k][i] = data3d[m][k][i];
        }
    }
}    
    ...
    for(int n = 0; n < n_processe; n++){
        ...
        if(my_id == 0){
            for(int m=0; m<mp; m++){
                for(int k = 0; k < NPZ[n]; k++){
                    for (int i=0; i<ny; i++)
                    {
                        data2d[m][NP[n] + k][i] = data_buff[m][k][i];
                    }
                }
            }
        }
        ...
        if(my_id != 0){
            MPI_Send(pdata_buff, ny*NPZ[my_id]*mp, MPI_DOUBLE, my_id-1, 1, MPI_COMM_WORLD);
        }

        if(my_id != n_processe-1){
            MPI_Recv(pdata_buff, ny*NPZ[my_id+1]*mp, MPI_DOUBLE, my_id+1, 1, MPI_COMM_WORLD, &status);
        }
    }
}
...
void Write_Savedata_timeaverage(int i0){
    ...
    MPI_Allgatherv( d0 , ny*NZ , MPI_DOUBLE , ppd0_buff , NPZ_in , NP_in, MPI_DOUBLE , MPI_COMM_WORLD);
    MPI_Allgatherv( u0 , ny*NZ , MPI_DOUBLE , ppu0_buff , NPZ_in , NP_in, MPI_DOUBLE , MPI_COMM_WORLD);
    MPI_Allgatherv( v0 , ny*NZ , MPI_DOUBLE , ppv0_buff , NPZ_in , NP_in, MPI_DOUBLE , MPI_COMM_WORLD);
    MPI_Allgatherv( w0 , ny*NZ , MPI_DOUBLE , ppw0_buff , NPZ_in , NP_in, MPI_DOUBLE , MPI_COMM_WORLD);
    MPI_Allgatherv( T0 , ny*NZ , MPI_DOUBLE , ppT0_buff , NPZ_in , NP_in, MPI_DOUBLE , MPI_COMM_WORLD);
    MPI_Allgatherv( P0 , ny*NZ , MPI_DOUBLE , ppP0_buff , NPZ_in , NP_in, MPI_DOUBLE , MPI_COMM_WORLD);
    sprintf(filename, "YZplane-time-average-%03d-tec.dat",i0);
    sprintf(filename1, "time1d-YZ%03d.dat",i0);
    ...
    if(my_id == 0){
        ...
        fprintf(fp1, "variables=tt,P1,P2,T1,T2,d1,d2,u1,u2,w1,w2\n");
        fprintf(fp1, "zone i=%d \n", mp);
        for(int i = 0; i < mp; i++){                               //这里时间换算为ms

            fprintf(fp1, "%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f%15.6f\n", 
            (tt[i]-tt[0])*9.35366*1e-4, P1d0[i]-ppd0_buff[0][0], P1d1[i]-ppd0_buff[m1][0], T1d0[i]-ppT0_buff[0][0], 
            T1d1[i]-ppT0_buff[m1][0], d1d0[i]-ppd0_buff[0][0], d1d1[i]-ppd0_buff[m1][0], u1d0[i]-ppu0_buff[0][0], 
            u1d1[i]-ppu0_buff[m1][0], w1d0[i]-ppw0_buff[0][0], w1d1[i]-ppw0_buff[m1][0]);
        }  
    ...
    if(my_id == 0){
        ...
        fp = fopen(filename, "w");
        fprintf(fp, "variables=x,y,z,drms,urms,vrms,wrms,Trms,Prms\n");//,d0,u0,v0,w0,T0,P0
        fprintf(fp, "zone i=%d ,j=%d ,k=%d\n", 1, ny, nz);
        fclose(fp); 
